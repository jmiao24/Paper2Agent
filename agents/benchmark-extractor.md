...
### Good Questions
- "First create the data fetcher: `EXAMPLE_DATA = pooch.create(path=pooch.os_cache('scverse_tutorials'), base_url='doi:10.6084/m9.figshare.22716739.v1/')` and load registry with `EXAMPLE_DATA.load_registry_from_doi()`. Then load bone marrow samples s1d1 and s1d3 using `EXAMPLE_DATA.fetch('s1d1_filtered_feature_bc_matrix.h5')` and `EXAMPLE_DATA.fetch('s1d3_filtered_feature_bc_matrix.h5')`, concatenate them. What is the shape (n_obs × n_vars) of the combined dataset before any filtering?"
- "Using the pbmc3k dataset from 10X Genomics (available via `sc.datasets.pbmc3k()`), filter cells with <200 genes and genes detected in <3 cells. Normalize to 10,000 counts per cell and log-transform. Run PCA with default settings. How many principal components are computed?"
